...
function renderCatManageList(){
  const html=CATS.map(c=>{
    ...
    return `<div style="display:flex;align-items:center;gap:8px;padding:9px 0;border-bottom:1px solid var(--brd)">
      ...
      <button class="fd-mini" onclick="openFormDesigner('${c.id}','${c.name.replace(/'/g,"\\'")}',false)" title="Formu düzenle">🛠️</button>
...
async function openEditCat(catId){
  if(!isAdmin()) return;
  const c=catById(catId); if(!c) return;
  const newName=await promptDialog({title:'Tür Adını Değiştir',message:'Yeni tür adı:',value:c.name,okText:'Kaydet'});
  if(newName===null) return;
  if(!newName.trim()){ toast('⚠️ Ad boş olamaz'); return; }
  const newIcon=await promptDialog({title:'İkon',message:'Emoji ikon (örn: 🧯 🔥 ⚡ 💧):',value:c.icon||'📦',okText:'Kaydet'});
  if(newIcon===null) return;
  // Tür varsayılan periyodu (bu türdeki tüm ekipmanlar için, kendi periyodu olanlar hariç)
  const curP=catDefaultPeriod(catId);
  const pStr=await promptDialog({
    title:'Varsayılan Kontrol Periyodu',
    message:'Bu türdeki ekipmanlar için varsayılan periyot (gün). Örnek: 7=haftada1, 30=ayda1, 90=3ayda1, 0=periyot yok. (Ekipmana özel periyot girilmişse o geçerli olur.)',
    value:String(curP), okText:'Kaydet'
  });
  if(pStr===null) return;
  let newPeriod=parseInt(pStr);
  if(isNaN(newPeriod)||newPeriod<0) newPeriod=curP;
  if(!S.catPeriods) S.catPeriods={};
  S.catPeriods[catId]=newPeriod;
  const base=isBaseCat(catId);
  if(base){
    // Temel tür — override ile sakla
    if(!S.catOverrides) S.catOverrides={};
    S.catOverrides[catId]={name:newName.trim(), icon:newIcon.trim()||'📦'};
  } else {
    const cc=S.customCats.find(x=>x.id===catId);
    if(cc){ cc.name=newName.trim(); cc.icon=newIcon.trim()||'📦'; }
  }
  rebuildCats();
  logActivity('cat_edit',`"${newName}" türü düzenlendi`);
  try{ await save(); populateCatSelects(); renderCatManageList(); toast('✅ Tür güncellendi'); }
  catch(e){ toast('❌ '+e.message,5000); }
}
  ...
  let html=`
    <div class="doc-tree-folder">
      <div class="doc-tree-row" style="padding-left:${9+pad}px" onclick="toggleDocNode('${node.id}')">
        ...
        ${node.manual?`<button class="doc-mini-btn" onclick="event.stopPropagation();uploadToCompanyFolder('${node.folderId}')" title="Belge Yükle" style="font-size:13px">⬆️</button>
    ...
    if(node.docs){
      node.docs.forEach(d=>{
        html+=`<div class="doc-tree-row" style="padding-left:${9+pad+20}px">
          ...
          ${node.manual?`<button class="doc-mini-btn" onclick="deleteCompanyDoc('${node.folderId}','${d.id}')" title="Sil" style="font-size:12px">🗑️</button>`:''}
        </div>`;
      });
    }
...
let _globalCats={ overrides:{}, forms:{}, custom:[] };
...
async function loadGlobalCats(){
  ...
  try{
    ...
    if(snap.exists()){
      const d=snap.val();
      _globalCats={ overrides:d.overrides||{}, forms:d.forms||{}, custom:toArr(d.custom||[]) };
    }
...
async function saveGlobalCats(){
  ...
  await _db.ref(`${TENANT_ROOT}/globalCats`).set({
    ...
    custom:_globalCats.custom||[], updatedAt:Date.now()
  });
}
...
function updateNotifBell(){
  ...
  if(!btn||!badge) return;
  if(S.cur?.isSuper || ROLE_LEVEL[S.cur?.role]>=3 || getUserPerms(S.cur||{}).includes('view_notifications') || getsMaintWarning(S.cur||{})){
    btn.style.display='';
...
function _oldRenderFbUsage_unused(){
  const el=document.getElementById('fb-usage-box');
  if(!el) return;
  if(!isAdmin()){ el.innerHTML=''; return; }

  // Tüm veriyi stringle, byte boyutunu hesapla
  const payload={users:S.users,mahals:S.mahals,equips:S.equips,reports:S.reports,logs:S.logs,activity:S.activity,notifications:S.notifications,customCats:S.customCats};
  const bytes=new Blob([JSON.stringify(payload)]).size;
  const LIMIT=1024*1024*1024; // 1 GB Spark limiti
  const pct=Math.min(100, (bytes/LIMIT)*100);
  const pctShow=pct<0.01?'<0.01':pct.toFixed(2);
  ...
  else if(pct>=70) barColor='linear-gradient(90deg,#f59e0b,#ef4444)';

  el.innerHTML=`<div class="ed-card" style="margin-bottom:18px">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
      <span style="font-size:13px;font-weight:700;color:var(--txt)">🔥 Firebase Veritabanı Doluluğu</span>
      <span style="font-size:12px;font-weight:700;color:${pct>=90?'var(--rtxt)':'var(--txt2)'}">%${pctShow}</span>
    </div>
    <div style="height:12px;background:var(--bg);border-radius:7px;overflow:hidden">
      <div style="height:100%;width:${Math.max(pct,0.5)}%;background:${barColor};transition:width .4s"></div>
    </div>
    <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--txt3);margin-top:6px">
      <span>${fmt(bytes)} kullanılıyor</span>
      <span>1 GB limit (ücretsiz plan)</span>
    </div>
    ${pct>=90?`<div style="margin-top:10px;padding:8px 12px;background:var(--rbg);color:var(--rtxt);border-radius:8px;font-size:12px;font-weight:600">⚠️ Veritabanı doluluğu kritik seviyede! Eski rapor/logları temizleyin.</div>`:''}
  </div>`;

  // %90 üstüyse ve bugün bildirim atılmadıysa otomatik bildirim
  if(pct>=90){
    const todayKey='dbfull-'+new Date().toISOString().slice(0,10);
    const already=S.notifications.find(n=>n.id===todayKey);
    if(!already){
      S.notifications.unshift({
        id:todayKey, reportId:null, equipName:'SİSTEM', mahalName:'Veritabanı',
        result:'fail', by:'Sistem', note:`Firebase doluluğu %${pctShow} — limite yaklaşıldı. Eski kayıtları temizleyin.`,
        date:nowStr(), ts:Date.now(), readBy:[],
      });
      save(); // sessizce kaydet
    }
  }
}
...
function renderMyIncomplete(){
  ...
  const mine=S.reports.filter(r=>r.incomplete && (r.byId ? r.byId===S.cur?.id : r.by===me));
  ...
  el.innerHTML=`<div style="background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.4);border-radius:var(--r12);padding:12px 14px;margin-bottom:14px">
    <div style="font-weight:700;color:var(--otxt);font-size:13.5px;margin-bottom:8px">⏳ Yarım kalan denetimleriniz (${mine.length})</div>
    ${slice.map(r=>`<div onclick="openReportDetail('${r.id}')" style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 10px;background:var(--card);border-radius:8px;margin-bottom:6px;cursor:pointer">
      <div style="min-width:0">
        ...
        <div style="font-size:11px;color:var(--txt3)">${safe(r.mahalName)} · ${r.date}</div>
      </div>
...
function _renderHomeRest(now){
  ...
  S.mahals.forEach((m,idx)=>{
    ...
    card.innerHTML=`
      ...
      ${isAdmin()?`<button class="mahal-edit-btn" data-mid="${m.id}">✏️</button>`:''}`;
...
function renderMahalPage(){
  ...
  document.getElementById('mahal-add-equip-wrap').style.display=canDo('add_equip')?'block':'none';
  const listEl=document.getElementById('mahal-equip-list');
  listEl.innerHTML=eq.length
    ?`<div class="equip-list">${eq.map(e=>equipRowHTML(e)).join('')}</div>`
    :`<div class="empty-state"><div class="empty-icon">🔧</div><p>Bu mahalde ekipman yok.</p></div>`;
  listEl.querySelectorAll('.equip-row').forEach(row=>row.addEventListener('click',()=>openEquipDetail(row.dataset.eid)));
}
...
function openInspModal(id){
  ...
  document.getElementById('insp-crits').innerHTML=crits.map((c,i)=>{
    ...
    return `<div class="crit-item${ans?' ans-'+ans:''}" id="ci-${i}">
      ...
      <div class="crit-btns">
        <button class="tog-btn${ans==='ok'?' ok-on':''}" id="cok-${i}"   onclick="setAns(${i},'ok','${c.replace(/'/g,'\\x27')}')">✅ Uygun</button>
        <button class="tog-btn${ans==='fail'?' fail-on':''}" id="cfail-${i}" onclick="setAns(${i},'fail','${c.replace(/'/g,'\\x27')}')">❌ Uygun Değil</button>
      </div>
    </div>`;}).join('');
...
function computeRiskData(){
  ...
  equips.forEach(e=>{
    ...
    const item={ name:e.name, cat:catName, mahal:mName, days:st.days, period:st.period };
...
function openRiskAnalysis(){
  ...
  const listSection=(title, items, color, emptyMsg)=>{
    if(!items.length) return '';
    return `<p class="sec-label" style="margin-top:14px;margin-bottom:6px">${title} (${items.length})</p>
      <div style="background:var(--bg);border:1px solid var(--brd);border-radius:10px;overflow:hidden;max-height:200px;overflow-y:auto">
        ${items.map(it=>`<div style="display:flex;align-items:center;gap:8px;padding:8px 11px;border-bottom:1px solid var(--brd)">
          ...
          ${it.days!=null?`<span style="font-size:11px;font-weight:700;color:${color}">${it.days} gün</span>`:''}
        </div>`).join('')}
      </div>`;
  };
  body.innerHTML=`
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">
      ...
      ${card('Yaklaşan', d.soon, '#ea580c')}
      ${card('Gecikmiş', d.overdue, 'var(--rtxt)', '%'+pct(d.overdue))}
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">
      ${card('Hiç Denetlenmemiş', d.never, '#6b7280')}
      ${card('Şu An Uygunsuz', d.failResult, 'var(--rtxt)')}
    </div>
    ...
    ${listSection('⏰ Gecikmiş Denetimler', d.overdueList, 'var(--rtxt)')}
    ${listSection('❌ Şu An Uygunsuz', d.failList, 'var(--rtxt)')}
    ${listSection('🔔 Yaklaşan Denetimler', d.soonList, '#ea580c')}
    ${listSection('⚪ Hiç Denetlenmemiş', d.neverList, '#6b7280')}
...
async function exportRiskPdf(){
  const d=computeRiskData();
  if(typeof window.jspdf==='undefined'){ toast('⚠️ PDF kütüphanesi yüklenemedi'); return; }
  const loading=showPersistentToast('📄 PDF hazırlanıyor…');
  try{
    const { jsPDF } = window.jspdf;
    const pdf=new jsPDF({unit:'pt',format:'a4'});
    const pw=pdf.internal.pageSize.getWidth();
    ...
    pdf.setFontSize(18); pdf.setTextColor(40);
    pdf.text('Risk Analizi Raporu', 40, y); y+=22;
    pdf.setFontSize(10); pdf.setTextColor(120);
    pdf.text((company?company+' · ':'')+new Date().toLocaleDateString('tr-TR'), 40, y); y+=24;
    // Özet kutusu
    pdf.setFontSize(11); pdf.setTextColor(40);
    const summary=[
      ['Toplam Ekipman', d.total],
      ['Uygun', d.ok],
      ['Yaklasan Denetim', d.soon],
      ['Gecikmis Denetim', d.overdue],
      ['Hic Denetlenmemis', d.never],
      ['Su An Uygunsuz', d.failResult],
    ];
    summary.forEach(([k,v])=>{ pdf.text(`${k}: ${v}`, 40, y); y+=18; });
    y+=10;
    const section=(title, items)=>{
      if(!items.length) return;
      if(y>740){ pdf.addPage(); y=50; }
      pdf.setFontSize(13); pdf.setTextColor(40); pdf.text(title+' ('+items.length+')', 40, y); y+=18;
      pdf.setFontSize(9); pdf.setTextColor(80);
      items.forEach(it=>{
        if(y>800){ pdf.addPage(); y=50; }
        const line=`• ${it.name} (${it.cat}) - ${it.mahal}`+(it.days!=null?` - ${it.days} gun`:'');
        pdf.text(line.substring(0,90), 48, y); y+=14;
      });
      y+=8;
    };
    section('Gecikmis Denetimler', d.overdueList);
    section('Su An Uygunsuz', d.failList);
    section('Yaklasan Denetimler', d.soonList);
    section('Hic Denetlenmemis', d.neverList);
    pdf.save(`Risk-Analizi-${new Date().toISOString().slice(0,10)}.pdf`);
    hidePersistentToast(loading);
    toast('✅ Risk analizi PDF indirildi');
  }catch(e){ hidePersistentToast(loading); toast('❌ PDF hatası: '+(e.message||''),5000); }
}
...
function renderReports(){
  ...
  if(wrap){
    const chips=[{id:'all',l:'Tümü'},{id:'ok',l:'✅ Uygun'},{id:'fail',l:'❌ Sorunlu'}];
  ...
  if(S.reportFilter!=='all') list=list.filter(r=>r.result===S.reportFilter);
  // Mahal filtresi
  ...
  document.getElementById('report-detail-container').innerHTML=`
    ...
    <div class="rpt-card">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        ${isAdmin()?`<button class="btn btn-danger btn-sm" onclick="deleteReport('${r.id}')">🗑️</button>`:''}
      </div>
    </div>`;
}
...
async function deleteReport(id){
  if(!isAdmin()){ toast('🚫 Sadece Admin silebilir'); return; }
...
function _unused_oldMailFailReports(){
  const failEquips=S.equips.filter(e=>getStatus(e)==='fail');
  if(!failEquips.length){ toast('✅ Uygunsuz ekipman yok'); return; }
  const byMahal={};
  failEquips.forEach(e=>{
    const mName=mahalById(e.mahalId)?.name||'Bilinmeyen Mahal';
    (byMahal[mName]=byMahal[mName]||[]).push(e);
  });
  const today=new Date().toLocaleDateString('tr-TR');
  let body=`UYGUNSUZ EKİPMAN RAPORU\n${today}\n`;
  body+='='.repeat(40)+'\n\n';
  body+=`Toplam ${failEquips.length} adet uygunsuz ekipman tespit edilmiştir:\n\n`;

  Object.entries(byMahal).forEach(([mahal,eqs])=>{
    body+=`\n📍 ${mahal} (${eqs.length} adet)\n`;
    body+='-'.repeat(30)+'\n';
    eqs.forEach(e=>{
      const cat=catById(e.cat);
      body+=`  • ${cat.icon} ${e.name} (${cat.name})\n`;
      // Son raporun uygunsuz kriterleri
      const lastRpt=S.reports.filter(r=>r.equipId===e.id).sort((a,b)=>b.createdAt>a.createdAt?1:-1)[0];
      if(lastRpt&&lastRpt.answers){
        const fails=Object.entries(lastRpt.answers).filter(([k,v])=>v==='fail').map(([k])=>k);
        fails.forEach(f=>body+=`      ⚠️ ${f}\n`);
      }
      if(e.cat==='tup-dolap'&&e.tupRows){
        e.tupRows.filter(t=>t.durum==='fail').forEach(t=>body+=`      ⚠️ Tüp ${t.tupNo}: ${t.sizinti==='evet'?'Sızıntı var':'Uygun değil'}\n`);
      }
      body+=`      Son denetim: ${e.lastInsp?e.lastInsp.date:'—'} / ${e.lastInsp?e.lastInsp.by:'—'}\n`;
    });
  });

  body+='\n\n'+'='.repeat(40)+'\n';
  body+='TakipEt Denetim Sistemi tarafından oluşturulmuştur.\n';

  const subject=`🚨 Uygunsuz Ekipman Raporu - ${today} (${failEquips.length} adet)`;
...
function unreadNotifCount(){
  if(!S.cur) return 0;
  if(!S.cur.isSuper && ROLE_LEVEL[S.cur.role]<3 && !getUserPerms(S.cur).includes('view_notifications') && !getsMaintWarning(S.cur)) return 0;
  return S.notifications.filter(n=>!(n.readBy||[]).includes(S.cur.id) && !(n.deletedBy||[]).includes(S.cur.id)).length;
}
...
function renderNotifications(){
  ...
  // Sadece bana görünür (silmediğim) bildirimler
  const notifs=S.notifications.filter(n=>!(n.deletedBy||[]).includes(S.cur?.id)).sort((a,b)=>b.ts-a.ts);
  const PER=10;
    ...
    html+=notifs.slice(start, start+PER).map(n=>{
      ...
      return `<div class="ed-card" style="margin-bottom:10px;border-left:3px solid ${accent};${unread?'':'opacity:.82'}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
          <div style="flex:1;min-width:0;cursor:pointer" onclick="markNotifRead('${n.id}').then(()=>{${n.reportId?`openReportDetail('${n.reportId}')`:n.equipId?`openEquipDetail('${n.equipId}')`:''}})">
...
/* Bir bildirimin alıcıları (bildirim alması gereken kullanıcı id'leri) */
function notifRecipients(n){
  // Bakım bildirimi: bakım uyarısı alanlar; diğerleri: admin + bildirim yetkisi olanlar
  if(n.type==='maintenance'){
    return S.users.filter(u=>u.isSuper || u.role==='admin' || getUserPerms(u).includes('maint_warn')).map(u=>u.id);
  }
  return S.users.filter(u=>u.isSuper || u.role==='admin' || getUserPerms(u).includes('view_notifications')).map(u=>u.id);
}
/* Bu bildirim bana görünür mü? (silmediysem) */
...
function openNewCatModal(){
  // İkon seçeneklerini render et
  const iconWrap=document.getElementById('newcat-icons');
  if(iconWrap){
    iconWrap.innerHTML=CAT_ICONS.map((ic,i)=>`<button type="button" class="newcat-ico-btn${i===0?' active':''}" data-ic="${ic}" onclick="selectNewCatIcon('${ic}',this)">${ic}</button>`).join('');
  }
  S.newCatIcon='📦';
  document.getElementById('newcat-name').value='';
  const ps=document.getElementById('newcat-period'); if(ps) ps.value='30';
  const pc=document.getElementById('newcat-period-custom'); if(pc){ pc.value=''; pc.style.display='none'; }
...
async function saveNewCat(){
  ...
  S.customCats.push({id, name, icon:S.newCatIcon||'📦'});
  // Başlangıç şablonu
  const tpl=document.getElementById('newcat-template')?.value||'blank';
  const tplForm=makeTemplateForm(tpl);
  if(tplForm){ if(!S.catForms)S.catForms={}; S.catForms[id]=tplForm; }
  // Varsayılan periyot
  const pSel=document.getElementById('newcat-period');
  if(pSel){
    let p;
    if(pSel.value==='custom'){ p=parseInt(document.getElementById('newcat-period-custom')?.value); if(isNaN(p)||p<1) p=30; }
    else p=parseInt(pSel.value);
    if(!isNaN(p)){ if(!S.catPeriods) S.catPeriods={}; S.catPeriods[id]=p; }
  }
  rebuildCats();
...
function openEditEquip(id){
  ...
  document.getElementById('btn-del-equip').style.display=isAdmin()?'':'none';
