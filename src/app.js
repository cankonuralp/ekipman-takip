...
function updateNotifBell(){
  ...
  if(!btn||!badge) return;
  if(S.cur?.isSuper || roleLevel(S.cur?.role)>=3 || getUserPerms(S.cur||{}).includes('view_notifications') || getsMaintWarning(S.cur||{})){
    btn.style.display='';
...
function isNotifVisibleToMe(n){
  ...
  return notifRecipients(n).includes(S.cur.id);  // sadece hedef kitledeysem
}
