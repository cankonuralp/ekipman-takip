...
function renderDocSearchResults(q, canManage){
  ...
  (S.mahals||[]).forEach(m=>{
    (S.equips||[]).filter(e=>e.mahalId===m.id).forEach(e=>{
      (e.documents||[]).forEach(d=>{ if((d.name||'').toLowerCase().includes(q)) rows.push({d, loc:m.name+' › '+e.name, manual:false}); });
    });
  });
...
async function deleteSelectedDocs(){
  if(!canDo('manage_docs')) return;
  if(!_docSelected.size){ toast('⚠️ Belge seçin'); return; }
  if(!await confirmDialog({title:'Belgeleri Sil',message:`${_docSelected.size} belge silinecek.`,danger:true,okText:'Sil'})) return;
  let del=0;
  for(const k of Array.from(_docSelected)){
    ...
    try{ if(doc.path) await _storage.ref(doc.path).delete(); }catch(e){}
