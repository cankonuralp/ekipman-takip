...
/* Genel evrağı seçilen şirkete GÖNDER → o şirkette "Gelenler" klasörüne düşer.
...
async function sendGlobalDocToCompany(fid, docId){
  ...
  const cid=await pickCompanyDialog('📤 Hangi şirkete gönderilsin?');
  if(!cid) return;
  showLoading(true);
  try{
    // Hedef şirketin companyFolders dalını çek (aktif olmayan şirkete de yazabilmek için)
    ...
    if(!inbox){ inbox={id:'cf'+Date.now(), name:'Gelenler', docs:[]}; folders.push(inbox); }
    ...
    toast(`✅ "${doc.name}" → ${c?.name||'şirket'} · Gelenler`);
    logSuperActivity('doc_push', `Genel evrak "${doc.name}" → ${c?.name||cid} (Gelenler klasörü)`);
