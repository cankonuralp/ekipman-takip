...
/* Şirket kataloğu dinleyicisi — girişten SONRA yeniden bağlanabilir olmalı:
   kilitli kurallarda anonim dinleyici iptal edilir; token alınınca tazelenir. */
function attachCompaniesListener(){
  if(!_companiesRef) return;
  if(_companiesListener){ try{ _companiesRef.off('value', _companiesListener); }catch(e){} }
  _companiesListener = _companiesRef.on('value', snap=>{
    ...
    // Girişsizken izin reddi NORMALDİR (kilitli kurallar) — giriş sonrası yeniden bağlanır
    console.warn('[Companies]', err.code||'', err.message||'');
  });
}
...
async function bindCompanyData(companyId){
  ...
  _listener = _ref.on('value', snap=>{
    ...
    // Kilitli kurallar: eski (token'sız) oturum veriye erişemez → temiz yeniden giriş iste
    if(String(err.code||err.message||'').toLowerCase().includes('permission')){
      try{ forceLogout('Güvenlik güncellemesi yapıldı — lütfen tekrar giriş yapın.'); }catch(e){}
      return;
    }
